...
public class GreedySolverEST_LRPT implements Solver {
	...
	private Task lrpt(Vector<Task> task, Instance instance, int [] duration_job, int [] duration_machine) {
		
		Vector<Task> est_tasks = est_lrpt(task,instance,duration_job, duration_machine);
		//System.out.println("est_task = " + est_tasks);
		Task est_min_lrpt = est_tasks.elementAt(0);
		
		for(int k=1; k<est_tasks.size(); k++) {
			if(getlong(instance,est_tasks.elementAt(k))>getlong(instance,est_min_lrpt)) {
	...
	private Vector<Task> est_lrpt(Vector<Task> task, Instance instance, int [] duration_job, int [] duration_machine){
	...
	public Result solve(Instance instance, long deadline) {
		...
		while(remainingTasks>0){
			Task current = lrpt(realisable,instance,duration_job, duration_machine);
