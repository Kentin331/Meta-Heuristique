...
public class TabooSolver implements Solver {
    ...
    public Result solve(Instance instance, long deadline) {
    	...
    	ResourceOrder initOrder = new ResourceOrder(sInit.schedule);
    	ResourceOrder currentOrder = initOrder.copy();
    	ResourceOrder bestOrder = initOrder.copy();
    	
    	int bestMakespan = currentOrder.toSchedule().makespan();
    	
    	int [][]sTaboo = new int[instance.numTasks*instance.numJobs][instance.numTasks*instance.numJobs];
    	int k = 0;    	
    	
    	while(k < 5000 && deadline - System.currentTimeMillis() > 1) {
    		k++;
	    	List<Block> lblock = blocksOfCriticalPath(currentOrder);	
	    	...
	    	for(Block b : lblock) {
	    		...
	    		for(Swap n : neig) {
	    			ResourceOrder tamp = currentOrder.copy();
	    			if(sTaboo[n.t1][n.t2] < k) {	    				
	    				n.applyOn(tamp); 
	    				currentOrder = tamp;
	    				
	    				if(currentOrder.toSchedule().makespan() < bestMakespan) {	    				
	    					bestOrder = currentOrder;    				
	    					bestMakespan = currentOrder.toSchedule().makespan();
	    					
	    				}  
	    				sTaboo[n.t2][n.t1] = k+dureeTaboo;
	    			}
	    		}    		
	    	}	    	
    ...
    List<Swap> neighbors(Block block) {
    	...
    	//sinon -> 2 swap aevc 1er voisin et last voisin
